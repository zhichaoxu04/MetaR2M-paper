# Apr 5, 2022: This function use  M ~ Cov + X.std and Y ~ Cov + X.std to do the isis
#              and Y ~ Cov and M.std and X.std to do the estimation
#
# Apr 20, 2022: We use the first half of dataset as the training which is the same as the package
#               idx1 <- 1:n/2
#
# Apr 21, 2022: Use M[idx1] & Y ~ M.std + X.std to do the isis to make sure we select the same M as package
#
# Jun 21, 2022: Use NEW variance to compute the CI
#
# Jul 20, whether set nsis = n/log(n), set is 072022 result, not set is 062422
#
# Jul 25, not set nsis and use seperate v.y.c and VYS.C to for Rsq and CI
# 092023: we use x instead of x.std to estimate
# 092723: we delete all the VYX-ish and keep v_yx to a neat result
# 101023: Add SOS and try to add HDMT for variable selection
# 101123: Add HDMT for variable selection


R2 <- function(Y, M, Covar, X, d, n, iter.max=3, nsis=NULL, first.half=TRUE, seed=2022, FDR=FALSE, FDRCutoff=0.2, method="iSIS", idx1=NULL){
  # ----- Load the packages
  library(GMMAT)
  library(SIS)
  library(tidyverse)
